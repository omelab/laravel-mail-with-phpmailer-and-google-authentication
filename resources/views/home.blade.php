...
@section('content')
    <div class="mt-2 text-gray-600 dark:text-gray-400 text-sm">
        ...
        <p>Once you click on below button it will redirect you to Google Account for select. Returned that with Token. If
            refresh Token Not found then please click Generate Token Button to grab token instead.</p>
    </div>
    <div class="mt-2 text-gray-600 dark:text-gray-400 text-sm flex justify-between">
        ...
        @if (session()->has('token'))
            <form action="{{ route('send.email') }}" method="post">
                ...
                <button type="submit" class="cursor p-2 px-6 bg-gray-100 text-gray-700 font-semibold">Send Test
                    Email</button>
            </form>
        @endif
    </div>
